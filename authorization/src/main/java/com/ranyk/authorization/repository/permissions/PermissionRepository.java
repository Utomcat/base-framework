...
public interface PermissionRepository extends JpaRepository<Permission, Long>, CrudRepository<Permission, Long>, JpaSpecificationExecutor<Permission> {
     ...
     */
    Boolean existsByCodeIn(List<String> permissionCodeList);
     ...
     * @return 返回查询结果 Boolean 值, true: 存在相同权限代码的数据; false: 不存在相同权限代码的数据;
     */
    Boolean existsByCodeInAndIdNotIn(List<String> permissionCodeList, List<Long> permissionIds);

    /**
     * 通过传入的权限 ID 列表, 修改权限信息表中对应的权限状态为指定状态
     *
     * @param ids        需要修改的权限 ID 列表
     * @param status     需要修改的权限状态值
     * @param updateId   修改人 ID
     * @param updateTime 修改时间
     * @return 修改的权限数量
     */
    @Modifying
    @Query("update Permission p set p.status = :status, p.updateId = :updateId, p.updateTime = :updateTime where p.id in :ids and p.status != -1")
    Integer updatePermissionStatusByIdIn(@Param("ids") List<Long> ids, @Param("status") Integer status, @Param("updateId") Long updateId, @Param("updateTime") LocalDateTime updateTime);
}
