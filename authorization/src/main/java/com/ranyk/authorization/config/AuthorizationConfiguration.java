...
public class AuthorizationConfiguration {
    ...
    public SaTokenConfig saTokenConfig() {
        ...
        saTokenConfig.setTokenName(authorizationConfigurationProperties.getTokenName());
        // token 有效期（单位：秒），默认30天，-1代表永不过期
        saTokenConfig.setTimeout(authorizationConfigurationProperties.getTimeout());
        // token 最低活跃频率（单位：秒），如果 token 超过此时间没有访问系统就会被冻结，默认-1 代表不限制，永不冻结
        saTokenConfig.setActiveTimeout(authorizationConfigurationProperties.getActivityTimeout());
        // 是否允许同一账号多地同时登录（为 true 时允许一起登录，为 false 时新登录挤掉旧登录）
        saTokenConfig.setIsConcurrent(authorizationConfigurationProperties.getIsConcurrent());
        // 在多人登录同一账号时，是否共用一个 token （为 true 时所有登录共用一个 token，为 false 时每次登录新建一个 token）
        saTokenConfig.setIsShare(authorizationConfigurationProperties.getIsShare());
        // token 风格
        saTokenConfig.setTokenStyle(authorizationConfigurationProperties.getTokenStyle());
        // 是否输出操作日志
        saTokenConfig.setIsLog(authorizationConfigurationProperties.getIsLog());
